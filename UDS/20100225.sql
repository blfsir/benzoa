       ...
       alter    proc UDS_StaffSearchPlan  --UDS_StaffSearchPlan  '个人计划','周计划','t','徐宁' 
...
select @PositionID=position_id from uds_staff a, uds_staff_in_position b where a.staff_id = b.staff_id and a.staff_name=+''+@PlanCreator+''
 ...
 SET @SQL = 'select id,plancreator+planyear+''年''+case planperiodtype when ''季计划'' then planperiod+''季度'' when ''月计划'' then planperiod+''月'' when ''周计划'' then planperiod+''周'' when ''年计划'' then ''度'' end +planobjecttype   as planname  from uds_plan where 1=1 '    
...
,UDS_Staff_In_Position  p where s.STAFF_ID=p.Staff_ID) a
 where a.position_id in (select position_id from uds_position where super_position_id= '''+@PositionID+''')))'  
